...
Bitmap^ CLRImageProcessing::ImageProcessing::TempleteMatching(Bitmap^ sourceBitmap, Bitmap^ templeteBitmap, int matchingRate)
{
	Bitmap^ targetBitmap = gcnew Bitmap(sourceBitmap);
	...
	//처리 시작
	SSETempleteMatching(sPtr, tPtr, templetePtr, sourceBitmap->Width, sourceBitmap->Height, templeteBitmap->Width, templeteBitmap->Height, matchingRate);
	//처리 끝
	sourceBitmap->UnlockBits(sourceBitmapData);
	targetBitmap->UnlockBits(targetBitmapData);
